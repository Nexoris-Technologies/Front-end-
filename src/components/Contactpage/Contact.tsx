...
function Contact() {
  ...
  const onSub = async (data:FormValues)=>{
      ...
      setThankYouURL(process.env.NODE_ENV === 'development'? 'http://localhost:3000/thank-you': 'https://nexoristech.com/thank-you');
        ...
        <div className="flex flex-col gap-[60px]">
          ...
          <div className="md:mb-[5rem] mb-[2rem]  flex flex-col lg:flex-row gap-[24px] px-4">
            ...
            <div className="flex-1 rounded-[16px] h-[700px] order-1 md:order-1 border border-[#D9D9D9] bg-[#D9D9D9]/4 p-6 flex flex-col gap-8">
              ...
              <form onSubmit={handleSubmit(onSub,onError)} noValidate action={`https://formsubmit.co/${targetEmail}`} method="POST">
              {/* Inputs */}
              <div className="flex flex-col gap-4">
                {/* First & Last Name */}
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex-1 flex flex-col gap-2">
                    ...
                    />
                    <label htmlFor="First Name">{typeof errors.firstname?.message !== "undefined"?`${errors.firstname?.message}`:`First Name`}</label>
                    <input
                      id="First Name"
                      type="text"
                      ...
                      {...register("firstname",{
                          ...
                          validate:{
                              firstNameLength: (fieldValue)=>(
                                  fieldValue.length > 3 || 'First Name must be at least 3 character long'
                              )
                          }
                      })}
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    <label htmlFor="Last Name">Last Name: {typeof errors.lastname?.message !== "undefined"?`${errors.lastname?.message}`:`${watch("lastname")}`}</label>
                    <input
                      id="Last Name"
                      type="text"
                      className="border border-[#8F9092]/40 px-3 py-2 rounded"
                      ...
                      {...register("lastname",{
                          ...
                          validate:{
                              lastNameLength: (fieldValue)=>(
                                  fieldValue.length > 3 || 'Last Name must be at least 3 character long'
                              )
                          }
                      })}
                    />
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex-1 flex flex-col gap-2">
                    <label htmlFor="Email">Email: {typeof errors.email?.message !== "undefined"?`${errors.email?.message}`:`${watch("email")}`}</label>
                    <input
                      id="Email"
                      type="email"
                      className="border border-[#8F9092]/40 px-3 py-2 rounded"
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    <label>Phone Number: {typeof errors.phonenumber?.message !== "undefined"?`${errors.phonenumber?.message}`:`${watch("phonenumber")}`}</label>
                    <input
                      type="text"
                      placeholder="+234"
                      className="border border-[#8F9092]/40 px-3 py-2 rounded"
                ...
                {/* Subject Select */}
                <div className="flex flex-col gap-2">
                  <label htmlFor="Select Subject">Select Subject: {(typeof errors._subject?.message !== "undefined" || watch("_subject")==="Select a subject")?`${errors._subject?.message}`:`${watch("_subject")}`}</label>
                ...
                {/* Message Textarea */}
                <div className="flex flex-col gap-2">
                  <label htmlFor="Message">Message: {typeof errors.message?.message !== "undefined"?`${errors.message?.message}`:``}</label>
                  <textarea
                    id="Message"
                ...
                <div className="mt-[1rem] w-[300px] mx-auto lg:mx-0  md:mt-[2rem]">
                  <Button className="bg-[#543CDA] text-white font-bold text-[16px] px-6 py-3 md:py-3 md:px-8 rounded lg:w-[250px] md:w-[200px]" type="submit">
                    Send Message
                  </Button>
                </div>
              </div>
