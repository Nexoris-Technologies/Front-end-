...
export async function generateMetadata(): Promise<Metadata> {
  const post = await getSingleType<TermsPageType>("terms-of-service");
  if (!post) return {};

  const { seoMeta, title, excerpt, featuredImage } = post;
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL;
  const canonical = `${siteUrl}/terms-of-service`;
...
export default async function TermsOfServicePage() {
  ...
  const { title, content, seoMeta, faqs, publishedAt, updatedAt } = post;

  const breadcrumbs = [
    { name: "Home", href: "/", isCurrentPage: false },
    { name: title, href: "/terms-of-service", isCurrentPage: true },
  ];
