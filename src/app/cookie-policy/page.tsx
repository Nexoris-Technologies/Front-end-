...
export async function generateMetadata(): Promise<Metadata> {
  const data = await getSingleType<CookiePolicyType>("cookie-policy");
  if (!data) return {};

  const { seoMeta, title, excerpt, featuredImage } = data;
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL;
  const canonical = `${siteUrl}/cookie-policy`;
...
// ✅ Page content with Breadcrumbs, FAQ, and JSON-LD schema
export default async function CookiePolicyPage() {
  ...
  const breadcrumbs = [
    ...
    { name: title, href: "/cookie-policy", isCurrentPage: true },
  ];
  ...
  return (
    <main className="max-w-3xl mx-auto py-10 px-4 mt-22">
